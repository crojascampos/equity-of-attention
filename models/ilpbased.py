...
from helpers.extra_functions import normalize_array, pos_bias, l1_norm_dist, dcg_score, fair_share_attention
...
class ILPBased:
    ...
    def __init__(
        ...
        self.rank_qty = len(self.rank_series)
        self.subj_qty = len(self.rank_series[0])
        self.pref_qty = 0
    ...
    def __iterate(
        ...
        'start' method.
        """
        ROWS = COLS = N = range(self.pref_qty)
        K = range(self.k)
        threshold = self.theta * self.ideal_score[rank]
    ...
    def prepare(
        ...
        self.pref_qty = min(pref_qty, self.subj_qty)
        ...
        for r in range(self.rank_qty):
            ...
            sorted_rank = argsort(-self.rank_series[r])
            for pos, index in enumerate(sorted_rank):
                r_pos[index] = pos
                if pos < self.k:
                    r_rel_subj[pos] = index

            self.pos[r] = r_pos
            self.rel_subj[r] = r_rel_subj

            # Get the ideal score of the ranking
    ...
    def start(
        ...
        for it in range(iters):
            ...
            for rank in range(self.rank_qty):
                ...
                for r in ROWS:
                    for c in COLS:
                        if value(solution_vars[r][c]) > 0.99:
                            result[r] = c
